...
export const DevicesIds: DeviceDefinition[] = [
    ...
    { vendorId: 0x0411, deviceId: 0x0083, name: 'Buffalo MD-HUSB' },
];
...
export class NetMD {
    ...
    private async _readReply(useFactoryCommand: boolean, overrideLength: number = -1) {
        ...
        if (overrideLength === -1) {
            ...
            let i = 0;
            while (len === 0) {
                await sleep(NetMD.readReplyRetryIntervalInMsec * Math.pow(2, i)); // Double wait time every attempt
                ({ len } = await this.getReplyLength());
                i++;
            }
        } else {
