...
export function sanitizeFullWidthTitle(title: string, justRemap: boolean = false) {
    ...
    // prettier-ignore
    const mappingsDE: { [key: string]: string } = { 'Ä': 'Ae', 'ä': 'ae', 'Ö': 'Oe', 'ö': 'oe', 'Ü': 'Ue', 'ü': 'ue', 'ß': 'ss' };

    let newTitle = title
        .split('')
        .map(n => mappingsJP[n] ?? n)
        .map(n => mappingsRU[n] ?? n)
        .map(n => mappingsDE[n] ?? n)
        .join('');
